...
import { LoginSplash } from "@/components/brand/login-splash";
...
function LoginForm() {
  ...
  async function onSubmit(formData: FormData) {
    ...
    if (session?.user.passwordExpired) {
      setNextPath(`${accountPath(session.user.role)}?expired=1`);
      return;
    }
    const fallback = session?.user.role === "viewer" ? "/portal" : "/dashboard";
    setNextPath(params.get("callbackUrl") || fallback);
  }

  return (
    <>
      {nextPath && <LoginSplash onDone={go} />}
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <form action={onSubmit} className="space-y-4">
            ...
            <Button className="w-full" disabled={pending || Boolean(nextPath)}>
